...
## ✨ 주요 기능
#### 1. 마을숙소 리스트, 예약일정 조회 등 구현 (Home.js/Search.js/ReservationPage.js)
- useState를 활용해 예약 상태를 관리하고 UI에 반영

#### 2. 숙소 페이지 UI 및 컴포넌트 구조 설계 (Search.js)
- 숙소 카드 컴포넌트를 분리하여 재사용 가능한 구조로 설계
- props를 활용해 숙소 정보 데이터를 전달

#### 3. map 기반 동적 리스트 렌더링 (rooms, experiences, reviews) (Detail.js)
- map()을 활용해 컴포넌트를 반복 렌더링하여 데이터 기반 UI 구성

#### 4. 객실 선택 및 원데이 클래스 추가 옵션 기능 구현 (Detail.js)
- 객실 및 추가 옵션 선택 시 useState로 선택 상태를 관리
- 선택된 옵션에 따라 가격 및 UI가 동적으로 변경

#### 5. 결제 버튼 클릭 시 결제 완료 화면 조건부 렌더링 & 예약저장 기능 (Detail.js)
...
<img width="1286" height="938" alt="조회리스트UI" src="https://github.com/user-attachments/assets/6884b538-4431-47f6-8b34-07742508acb0" />
<img width="1278" height="739" alt="필터리스트01" src="https://github.com/user-attachments/assets/d7475eee-6963-4e57-90dd-00a1e2b8e1ff" />
...
💡 dateRange의 초기값을 null 또는 적절한 범위로 설정하여, 체크인만 선택해도 캘린더가 닫히지 않도록 수정.<br />
💡 onSelect 이벤트에서 from과 to가 모두 존재할 때만 캘린더를 닫도록 조건을 추가.<br />
👉 체크인만 선택해도 캘린더가 유지되고, 체크인과 체크아웃 범위가 모두 선택되었을 때만 자동으로 닫히도록 정상 동작<br />
